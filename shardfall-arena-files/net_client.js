...
startRecall = h => { if (!NET.on) return _startRecall(h); if (mine(h)) netSend({ t: 'in', a: 'recall' }); };
...
function netHookInput() {
  addEventListener('keydown', e => {
    ...
    else if (k === 'H') { netSend({ t: 'in', a: 'hold' }); G.amoveArmed = false; e.stopPropagation(); }
    else if (k === 'S') { netSend({ t: 'in', a: 'stop' }); G.amoveArmed = false; e.stopPropagation(); }
  }, true);
