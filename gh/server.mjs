...
function startMatch(room) {
  ...
  room.known = new Map(); room.sent = new Map();
  room.events = [];
...
function resetRoom(room) {           // back to the lobby after a match ends
  ...
  room.known = new Map(); room.sent = new Map(); room.events = []; room.seq = 0; room.ticks = 0;
...
function sendSnapshots(room) {
  const api = room.sim, G = api.G;
  // only units whose row actually changed travel: towers, idle pets and dead
  // minions cost nothing, which is most of the map in a quiet moment
  const units = [], spawns = [], seen = new Set();
  room.sent = room.sent || new Map();
  for (const u of G.units) {
    seen.add(u.id);
    if (!room.known.has(u.id)) { room.known.set(u.id, 1); if (u.kind !== 'hero') spawns.push(spawnRow(u)); }
    const row = unitRow(u, G), key = row.join(',');
    if (room.sent.get(u.id) === key) continue;
    room.sent.set(u.id, key);
    units.push(row);
  }
  const removed = [];
  for (const id of room.known.keys()) if (!seen.has(id)) { removed.push(id); room.known.delete(id); room.sent.delete(id); }
    ...
    switch (m.t) {
      ...
      case 'join': {
        ...
        if (r.started) {           // rejoin: take back your own champion
          ...
          r.known = new Map(); r.sent = new Map();   // resend every unit description to everyone
