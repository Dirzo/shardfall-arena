...
function update(dt) {
  ...
  if (!G.voidmaw.unit && G.t >= G.voidmaw.next) {
    ...
    announce('Voidmaw has emerged', 'Its web crowns the top of the map — slay it to empower your minions');
...
function pickMinionTarget(m) {
  ...
  for (const u of G.units) {
    ...
    const d = Math.hypot(u.x - m.x, u.y - m.y); if (d > 400 + u.r) continue;
    let pri;
